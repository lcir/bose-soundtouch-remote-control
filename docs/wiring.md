...
Obrazek ukazuje cely pinout `LOLIN/Wemos S2 Mini`. Jen u pinu pouzitych v tomto projektu je dopsane konkretni zapojeni `OLED`, enkoderu, tlacitka encoderu a stavove LED.
...
| Tlačítko encoderu `SW` | `GPIO11` | aktivní v `LOW`, zároveň servisní hold při bootu |
| Stavová LED | `GPIO18` | výstup pro LED |
...
Vestavěné tlačítko encoderu je zapojené jako jednoduchý spínač proti zemi.
...
Firmware počítá s jednou samostatně řízenou LED.

| LED kontakt | Připojit na |
|---|---|
| `Anoda` | `GPIO18` -> samostatny seriovy rezistor -> LED |
| `Katoda` | `GND` |

Poznámka:

- firmware má výchozí logiku `active HIGH`
- pokud použiješ jiný aktivní stav, změň `STATUS_LED_ACTIVE_HIGH` v [PinConfig.h](/Users/peny/Development/Projects/boser-remote-control/include/PinConfig.h)
- rezistor patri mezi `GPIO18` a LED
